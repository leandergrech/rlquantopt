...
from rlquantopt.metarl_agents.sync_vector_env import SyncVectorEnv
from rlquantopt.metarl_agents.batch_episodes import BatchEpisodes
# from maml_rl.samplers.sampler import Sampler, make_env
from rlquantopt.metarl_agents.utils import reinforce_loss
...
class MultiTaskSampler(Sampler):
    """Vectorized sampler to sample trajectories from multiple environments.
    ...
    experiment_folder : str
        Path to the folder where the experiment is saved.

    experiment_type : str
        Type of experiment, usually "train" or "test".
    ...
    def sample_async(self, tasks, **kwargs):
        if self._waiting_sample:
            raise RuntimeError(
                "Calling `sample_async` while waiting "
    ...
    def sample_wait(self, episodes_futures):
        if not self._waiting_sample:
            raise RuntimeError(
                "Calling `sample_wait` without any " "prior call to `sample_async`."
            )
    ...
    def _start_consumer_threads(self, tasks, num_steps=1):
        ...
        train_episodes_futures = [
            [self._event_loop.create_future() for _ in tasks] for _ in range(num_steps)
        ]
...
class SamplerWorker(mp.Process):
    def __init__(
        ...
        self.train = env_kwargs["train"]
        self.experiment_folder = experiment_folder
        self.experiment_type = experiment_type

        self.number_interact = 0

    def sample(
        self, index, num_steps=1, fast_lr=0.5, gamma=0.95, gae_lambda=1.0, device="cpu"
    ):
        ...
        # for optimization.update_params

        params = None
        directory = os.path.join(self.experiment_folder, self.experiment_type)

        if not self.train:
            path_to_file = os.path.join(directory, f"policy_{index}.npy")
        ...
        for step in range(num_steps):
            ...
            if step == num_steps - 1:  # save policy at the last step
                with open(
                    os.path.join(directory, "policy_" + str(index) + ".npy"), "wb"
                ) as f:
                    f.truncate(0)
                    # print("Saved policy for Task " + str(index))
    ...
    def sample_trajectories(self, params=None):
        ...
        with torch.no_grad():
            while not (self.envs._terminateds.all() or self.envs._truncateds.all()):
                ...
                action_list.append(actions_tensor.numpy())
                ...
                if len(infos["infos"]) > 0:
                    tasks = infos["infos"][0]["task"]
                    times = infos["infos"][0]["time"]
                else:
                    tasks = infos["infos"]
                    times = infos["infos"]

                # self.number_interact+=
                rewards_cp = deepcopy(rewards)
                batch_id_cp = deepcopy(batch_ids)
                tasks_cp = deepcopy(tasks)
                reward_list.append(rewards_cp)
                batch_ids_list.append(batch_id_cp)
                task_list.append(tasks_cp)
                times_list.append(times)
                done_list.append(terminateds_ | truncateds_)
                # print(done_list)
