...
from rlquantopt.metarl_agents.utils import reinforce_loss, detach_distribution, to_numpy, vector_to_parameters, weighted_mean, conjugate_gradient
...
class MAMLTRPO(GradientBasedMetaLearner):
    ...
    def hessian_vector_product(self, kl, damping=1e-2):
        ...
        def _product(vector, retain_graph=True):
            ...
            grad2s = torch.autograd.grad(
                grad_kl_v, self.policy.parameters(), retain_graph=retain_graph
            )
    ...
    async def surrogate_loss(self, train_futures, valid_futures, old_pi=None):
        ...
        with torch.set_grad_enabled(old_pi is None):
            ...
            kls = weighted_mean(
                kl_divergence(pi, old_pi), lengths=valid_episodes.lengths
            )
    ...
    def step(
        ...
        grads = torch.autograd.grad(
            old_loss, self.policy.parameters(), retain_graph=True
        )
        ...
        for _ in range(ls_max_steps):
            vector_to_parameters(
                old_params - step_size * step, self.policy.parameters()
            )

            losses, kls, _ = self._async_gather(
                [
                    self.surrogate_loss(train, valid, old_pi=old_pi)
                    for (train, valid, old_pi) in zip(
                        zip(*train_futures), valid_futures, old_pis
                    )
                ]
            )
